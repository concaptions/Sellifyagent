...
| Gmail read | ✅ Live, verified. `read_emails` = search + ~200-char snippet; `read_email(id)` = full body (plain or HTML→text, ≤12k chars, attachment names) — added 2026-09-24 after the client hit a cut-off date; **live check of `read_email` pending** |
