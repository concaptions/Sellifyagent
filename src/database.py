...
def init_database():
    with get_db() as conn:
        ...
        cur.execute("""
            ...
            CREATE TABLE IF NOT EXISTS pa_chat_history (
                id SERIAL PRIMARY KEY,
                user_id TEXT NOT NULL,
                role TEXT NOT NULL,
                content TEXT NOT NULL,
            ...
            CREATE INDEX IF NOT EXISTS idx_pa_users_phone ON pa_users(phone);
            CREATE INDEX IF NOT EXISTS idx_pa_notes_user ON pa_notes(user_id);
            CREATE INDEX IF NOT EXISTS idx_pa_chat_history_user ON pa_chat_history(user_id);
        """)
        conn.commit()
...
def upsert_user(phone: str, name: str | None = None) -> dict:
    with get_db() as conn:
        ...
        cur.execute("""
            INSERT INTO pa_users (phone, name, message_count, last_msg_at)
            VALUES (%s, %s, 1, NOW())
            ON CONFLICT (phone) DO UPDATE SET
                message_count = pa_users.message_count + 1,
                last_msg_at = NOW(),
                updated_at = NOW()
            RETURNING *
        """, (phone, name))
...
def get_user(phone: str) -> dict | None:
    with get_db() as conn:
        cur = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        cur.execute("SELECT * FROM pa_users WHERE phone = %s", (phone,))
        row = cur.fetchone()
...
def save_note(user_id: str, title: str, content: str, tags: list[str] | None = None) -> int:
    with get_db() as conn:
        cur = conn.cursor()
        cur.execute("""
            INSERT INTO pa_notes (user_id, title, content, tags)
            VALUES (%s, %s, %s, %s)
            RETURNING id
...
def save_chat_message(user_id: str, role: str, content: str, tool_calls: dict | None = None):
    with get_db() as conn:
        cur = conn.cursor()
        cur.execute("""
            INSERT INTO pa_chat_history (user_id, role, content, tool_calls)
            VALUES (%s, %s, %s, %s)
